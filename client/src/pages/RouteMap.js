...
//     // Define waypoints for A, B, and C
//     const waypoints = [
...
//     // Calculate centroid of the three waypoints
//     const calculateCentroid = () => {
//         const totalLat = waypoints.reduce((sum, wp) => sum + wp.lat, 0);
//         const totalLon = waypoints.reduce((sum, wp) => sum + wp.lon, 0);
//         return {
//             lat: (totalLat / waypoints.length).toFixed(6),
//             lon: (totalLon / waypoints.length).toFixed(6)
//         };
//     };
...
//             // Map bounds and zoom settings
//             const southWest = L.latLng(22.100, 87.100);
//             const northEast = L.latLng(22.500, 87.500);
//             const bounds = L.latLngBounds(southWest, northEast);

//             // Initialize map at centroid with zoom level 15
...
//             // Update cursor coordinates dynamically
...
//             // Plot waypoints and load map layers
...
//         // Direct A → C path
//         L.polyline([[waypoints[0].lat, waypoints[0].lon], [waypoints[2].lat, waypoints[2].lon]], { color: 'green', weight: 3, dashArray: '5,5' }).addTo(pathLayer.current);
//     };
...
//             const minZoom = 12; // Minimum zoom level
//             const baseSize = 120; // Maximum size at max zoom
    
//             // Calculate size based on zoom (smaller size for zoom out)
//             const size = baseSize * ((currentZoom - minZoom) / (maxZoom - minZoom));
//             const finalSize = Math.max(40, size); // Ensure it doesn't shrink too much
    
//             return { size: finalSize, angle };
//         };
    
//         const { size } = updateArrowSize();
...
//         const arrowMarker = L.marker([(start.lat + end.lat) / 2, (start.lon + end.lon) / 2], { icon: arrowIcon }).addTo(pathLayer.current);
    
//         // Update arrow size on zoom
...
//             L.marker([alert.lat, alert.lon], { icon: alertIcon }).addTo(mapRef.current);
//         });
//     };
...
//             L.marker([annotation.lat, annotation.lon], { icon: annotationIcon }).addTo(mapRef.current);
//         });
//     };

//     return (
//         <div style={{ textAlign: 'center', marginTop: '20px' }}>
//             <h2>Route Map with Waypoints (A → B → C)</h2>
        ...
        if (!mapRef.current) {
            ...
            const geocoder = L.Control.geocoder({
                ...
                if (searchMarker.current) {
                    mapRef.current.removeLayer(searchMarker.current);
                }
                searchMarker.current = L.marker(latlng).addTo(mapRef.current)
                    .bindPopup(`Location: ${e.geocode.name}`)
                    .openPopup();
            }).addTo(mapRef.current);
